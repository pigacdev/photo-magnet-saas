...
export type OrderImageDeleteConfirmModalProps = {
  open: boolean;
  imageCount: number;
  saving: boolean;
  onClose: () => void;
...
export function OrderImageDeleteConfirmModal({
  open,
  imageCount,
  saving,
  onClose,
  onConfirm,
}: OrderImageDeleteConfirmModalProps) {
  useEffect(() => {
    if (!open) return;
      ...
      >
        ...
        <div className="space-y-3 px-4 py-4 sm:px-5">
          <p className="text-sm text-muted-foreground">
            Permanently delete {label} from this order? The files will be removed
            and the image slots will show as removed. This cannot be undone.
          </p>
        </div>
